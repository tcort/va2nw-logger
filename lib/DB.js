...
class DB {
    ...
    stats(callback) {
        ...
        async.map([
            ...
            "SELECT COUNT(*) QSO_TOTAL, COUNT(DISTINCT CALL) QSO_UNIQUE_CALL_TOTAL, COUNT(DISTINCT SKCC) UNIQUE_SKCC_NR, COUNT(SKCC <> '') QSO_SKCC_TOTAL, COUNT(MY_POTA_REF <> '' OR POTA_REF <> '') QSO_POTA_TOTAL, COUNT(CONTEST_ID <> '') QSO_CONTEST_TOTAL FROM logs;",
            ...
            "SELECT COUNT(*) QSO_QRP_TOTAL FROM logs WHERE CAST(TX_PWR AS REAL) <= 5;",
            ...
            const [ bands, modes, total_qso, years, months, daysofweek, frequent_flyers, bookends, pota_activations, qrp_totals, qrpp_totals, my_rigs, my_antennas, contest_totals ] = results;
            ...
            const stats = {
                ...
                nqrp: qrp_totals[0].QSO_QRP_TOTAL,
                nqrpp: qrpp_totals[0].QSO_QRPP_TOTAL,
