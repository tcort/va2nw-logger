 ...
 * operator - CAll sign of the operator of the loggin station - ADIF Fields: OPERATOR
 ...
 * pfx - callsign prefix of 'callsign'. ADIF Fields: PFX
 * my_spc - State/Province/Country of contacted station. ADIF Fields: MY_STATE
 * dxcc - DXCC entity code. ADIF Fields: DXCC
 * my_dxcc - my DXCC entity code. ADIF Fields: MY_DXCC
 * country - country name. ADIF Fields: COUNTRY
 * my_country - my country name. ADIF Fields: MY_COUNTRY
 * gridsquare - maidenhead gridsquare. ADIF Fields: GRIDSQUARE
 * my_gridsquare - maidenhead gridsquare. ADIF Fields: MY_GRIDSQUARE
 * cq_zone - CQ Zone. ADIF Fields: CQZ
 * my_cq_zone - CQ Zone. ADIF Fields: MY_CQZ
 * itu_zone - ITU Zone. ADIF Fields: ITUZ
 * my_itu_zone - ITU Zone. ADIF Fields: MY_ITUZ
 * qsl_sent - confirmation sent. ADIF Fields: QSL_SENT
 * qsl_sent_date - date confirmation sent. ADIF Fields: QSLSDATE
 * qsl_rcvd - confirmation received. ADIF Fields: QSL_RCVD
 * qsl_rcvd_date - date confirmation received. ADIF Fields: QSLRDATE
 * sota - contacted station's SOTA reference. ADIF Fields: SOTA_REF, SIG, SIG_INFO
 * my_sota - logging station's SOTA reference. ADIF Fields: MY_SOTA_REF, MY_SIG, MY_SIG_INFO
 * pota - contacted station's POTA reference. ADIF Fields: POTA_REF, SIG, SIG_INFO
 * my_pota - logging station's POTA reference(s). ADIF Fields: MY_POTA_REF, MY_SIG, MY_SIG_INFO
...
class DB {
    ...
    #applySchemaChangeOperator(table_info, callback) {

        // check if operator column exists
        if (table_info.some(table_info => table_info.name === 'operator')) {
            return callback(); // exists, nothing to do here
        }

        // does not exist, add it
        this.#conn.run('ALTER TABLE qsos ADD COLUMN operator TEXT AFTER station_callsign;', [], err => {
            if (err) {
                return callback(err);
            }
    ...
    #applySchemaChangeLoTW(table_info, callback) {
        ...
        async.eachSeries(fields, (field, callback) => {
            ...
            if (table_info.some(table_info => table_info.name === `${field}`)) {
                return callback(); // exists, nothing to do here
            }

            // does not exist, add it
            this.#conn.run(`ALTER TABLE qsos ADD COLUMN ${field} TEXT;`, [], err => {
                if (err) {
                    return callback(err);
                }
    ...
    #applySchemaChangeOTA(table_info, callback) {
    ...
    #applySchemaChangeContest(table_info, callback) {

        // check if contacted_op column exists
        if (table_info.some(table_info => table_info.name === 'contest')) {
            return callback(); // exists, nothing to do here
        }

        // does not exist, add it
        this.#conn.run('ALTER TABLE qsos ADD COLUMN contest TEXT;', [], err => {
            if (err) {
                return callback(err);
            }
                        ...
                        this.#applySchemaChangeSpc(table_info, err => {
                            ...
                            this.#applySchemaChangeContactedOp(table_info, err => {
                                ...
                                this.#applySchemaChangeFists(table_info, err => {
                                    ...
                                    this.#applySchemaChangeOperator(table_info, err => {
                                        if (err) {
                                            return callback(err);
                                        }

                                        this.#applySchemaChangeLoTW(table_info, err => {
                                            if (err) {
                                                return callback(err);
                                            }
                                            this.#applySchemaChangeOTA(table_info, err => {
                                                if (err) {
                                                    return callback(err);
                                                }
                                                this.#applySchemaChangeContest(table_info, err => {
                                                    if (err) {
                                                        return callback(err);
                                                    }
                                                    callback();
    ...
    qsoInsert(qso, callback) {
        ...
        const sql = 'INSERT INTO qsos (timeon, timeoff, frequency, mode, power, callsign, rst_sent, rst_rcvd, remarks, station_callsign, operator, spc, name, skcc, fists, contacted_op, pfx, my_spc, dxcc, my_dxcc, country, my_country, gridsquare, my_gridsquare, cq_zone, my_cq_zone, itu_zone, my_itu_zone, qsl_sent, qsl_sent_date, qsl_rcvd, qsl_rcvd_date, sota, my_sota, pota, my_pota, contest) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);';
        const vals = [ qso.timeon, qso.timeoff, qso.frequency, qso.mode, qso.power, qso.callsign, qso.rst_sent, qso.rst_rcvd, qso.remarks, qso.station_callsign, qso.operator, qso.spc, qso.name, qso.skcc, qso.fists, qso.contacted_op, qso.pfx, qso.my_spc, qso.dxcc, qso.my_dxcc, qso.country, qso.my_country, qso.gridsquare, qso.my_gridsquare, qso.cq_zone, qso.my_cq_zone, qso.itu_zone, qso.my_itu_zone, qso.qsl_sent, qso.qsl_sent_date, qso.qsl_rcvd, qso.qsl_rcvd_date, qso.sota, qso.my_sota, qso.pota, qso.my_pota, qso.contest ];
    ...
    qsoUpdate(qso_id, qso, callback) {
        ...
        const sql = 'UPDATE qsos SET timeon = ?, timeoff = ?, frequency = ?, mode = ?, power = ?, callsign = ?, rst_sent = ?, rst_rcvd = ?, remarks = ?, station_callsign = ?, operator = ?, spc = ?, name = ?, skcc = ?, fists = ?, contacted_op = ?, pfx = ?, my_spc = ?, dxcc = ?, my_dxcc = ?, country = ?, my_country = ?, gridsquare = ?, my_gridsquare = ?, cq_zone = ?, my_cq_zone = ?, itu_zone = ?, my_itu_zone = ?, qsl_sent = ?, qsl_sent_date = ?, qsl_rcvd = ?, qsl_rcvd_date = ?, sota = ?, my_sota = ?, pota = ?, my_pota = ?, contest = ? WHERE qso_id = ?';
        const vals = [ qso.timeon, qso.timeoff, qso.frequency, qso.mode, qso.power, qso.callsign, qso.rst_sent, qso.rst_rcvd, qso.remarks, qso.station_callsign, qso.operator, qso.spc, qso.name, qso.skcc, qso.fists, qso.contacted_op, qso.pfx, qso.my_spc, qso.dxcc, qso.my_dxcc, qso.country, qso.my_country, qso.gridsquare, qso.my_gridsquare, qso.cq_zone, qso.my_cq_zone, qso.itu_zone, qso.my_itu_zone, qso.qsl_sent, qso.qsl_sent_date, qso.qsl_rcvd, qso.qsl_rcvd_date, qso.sota, qso.my_sota, qso.pota, qso.my_pota, qso.contest, qso_id ];
