...
class DB {
    ...
    select(query, callback) {
        ...
        const sql = (query?.order === 'asc' ?
            'SELECT * FROM logs WHERE (TIMESTAMP BETWEEN ? AND ?) AND (CALL LIKE ?) AND (STATION_CALLSIGN LIKE ?) ORDER BY APP_TCADIF_QSO_ID ASC LIMIT ?;' :
            'SELECT * FROM logs WHERE (TIMESTAMP BETWEEN ? AND ?) AND (CALL LIKE ?) AND (STATION_CALLSIGN LIKE ?) ORDER BY APP_TCADIF_QSO_ID DESC LIMIT ?;');
        const vals = [
