...
class DB {
    ...
    qsoSelect(query, callback) {
        ...
        const sql = (query?.order === 'asc' ?
            'SELECT * FROM qsos WHERE (timeon BETWEEN ? AND ?) AND (callsign LIKE ?) ORDER BY timeon ASC LIMIT ?, ?;' :
            'SELECT * FROM qsos WHERE (timeon BETWEEN ? AND ?) AND (callsign LIKE ?) ORDER BY timeon DESC LIMIT ?, ?;');
        const vals = [
            query.since ?? '1900-01-01T00:00:00.000Z',
            query.before ?? '2099-12-31T23:59:59.000Z',
            typeof query.callsign === 'string' && query.callsign !== '' ? query.callsign : '%',
