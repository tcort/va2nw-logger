...
class DB {
    ...
    qsoInsert(qso, callback) {
        ...
        const sql = 'INSERT INTO qsos (timeon, timeoff, frequency, mode, power, callsign, rst_sent, rst_rcvd, qsl_sent, qsl_rcvd, remarks, station_callsign) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);';
        const vals = [ qso.timeon, qso.timeoff, qso.frequency, qso.mode, qso.power, qso.callsign, qso.rst_sent, qso.rst_rcvd, qso.qsl_sent, qso.qsl_rcvd, qso.remarks, qso.station_callsign ];
    ...
    qsoUpdate(qso_id, qso, callback) {
        ...
        const sql = 'UPDATE qsos SET timeon = ?, timeoff = ?, frequency = ?, mode = ?, power = ?, callsign = ?, rst_sent = ?, rst_rcvd = ?, qsl_sent = ?, qsl_rcvd = ?, remarks = ?, station_callsign = ? WHERE qso_id = ?';
        const vals = [ qso.timeon, qso.timeoff, qso.frequency, qso.mode, qso.power, qso.callsign, qso.rst_sent, qso.rst_rcvd, qso.qsl_sent, qso.qsl_rcvd, qso.remarks, qso.station_callsign, qso_id ];
    ...
    qsoCount(query, callback) {
        ...
        const sql = 'SELECT COUNT(*) AS count FROM qsos WHERE (timeon BETWEEN ? AND ?) AND (callsign LIKE ?);';
        const vals = [
    ...
    qsoSelect(query, callback) {
        ...
        const sql = (query?.order === 'asc' ?
            'SELECT * FROM qsos WHERE (timeon BETWEEN ? AND ?) AND (callsign LIKE ?) AND (IFNULL(STATION_CALLSIGN,"") LIKE ?) ORDER BY timeon ASC LIMIT ?, ?;' :
            'SELECT * FROM qsos WHERE (timeon BETWEEN ? AND ?) AND (callsign LIKE ?) AND (IFNULL(STATION_CALLSIGN,"") LIKE ?) ORDER BY timeon DESC LIMIT ?, ?;');
        const vals = [
            query.since ?? '1900-01-01T00:00:00.000Z',
            query.before ?? '2099-12-31T23:59:59.000Z',
            typeof query.callsign === 'string' && query.callsign !== '' ? query.callsign : '%',
