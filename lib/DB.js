...
class DB {
    ...
    remove(APP_TCADIF_QSO_ID, callback) {

        const sql = 'DELETE FROM logs WHERE APP_TCADIF_QSO_ID = ?;';
    ...
    stationCallsigns(callback) {
        const sql = 'SELECT DISTINCT STATION_CALLSIGN FROM logs';
        const vals = [ ];
    ...
    selectCount(query, callback) {
        ...
        const sql = 'SELECT COUNT(*) AS count FROM logs WHERE (TIMESTAMP BETWEEN ? AND ?) AND (CALL LIKE ?) AND (IFNULL(STATION_CALLSIGN,"") LIKE ?);';
        const vals = [
    ...
    select(query, callback) {
        ...
        const sql = (query?.order === 'asc' ?
            'SELECT * FROM logs WHERE (TIMESTAMP BETWEEN ? AND ?) AND (CALL LIKE ?) AND (IFNULL(STATION_CALLSIGN,"") LIKE ?) ORDER BY APP_TCADIF_QSO_ID ASC LIMIT ?, ?;' :
            'SELECT * FROM logs WHERE (TIMESTAMP BETWEEN ? AND ?) AND (CALL LIKE ?) AND (IFNULL(STATION_CALLSIGN,"") LIKE ?) ORDER BY APP_TCADIF_QSO_ID DESC LIMIT ?, ?;');
        const vals = [
            query.since ?? '1900-01-01T00:00:00Z',
            query.before ?? '2099-12-31T23:59:59Z',
            typeof query.callsign === 'string' && query.callsign !== '' ? query.callsign : '%',
    ...
    appTcadifMyKeyInfoStartsWith(partial, callback) {
        const sql = 'SELECT DISTINCT APP_TCADIF_MY_KEY_INFO FROM logs WHERE APP_TCADIF_MY_KEY_INFO LIKE ? LIMIT 10';
        const vals = [ partial + '%' ];
        ...
        this.#conn.all(sql, vals, (err, rigs) => {
            ...
            callback(null, rigs.map(row => row.APP_TCADIF_MY_KEY_INFO));
        });
    }
    ...
    #time_human(qso) {
        return moment.utc(this.#createTimestamp(qso)).format('HH:mm:ss');
    }
    ...
    #fixupQso(qso) {
        ...
        qso = qso instanceof QSO ? qso.toObject() : (new QSO(qso)).toObject();
