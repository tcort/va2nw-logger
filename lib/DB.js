...
class DB {
    ...
    stats(callback) {
        ...
        async.map([
            ...
            "SELECT COUNT(*) QSO_TOTAL, COUNT(DISTINCT CALL) QSO_UNIQUE_CALL_TOTAL, COUNT(CONTEST_ID <> '') QSO_CONTEST_TOTAL FROM logs;",
            ...
            "SELECT COUNT(MY_POTA_REF <> '' OR POTA_REF <> '') QSO_POTA_TOTAL, COUNT(MY_POTA_REF <> '') QSO_POTA_ACTIVATOR_TOTAL, COUNT(POTA_REF <> '') QSO_POTA_HUNTER_TOTAL FROM logs WHERE MY_POTA_REF <> '' OR POTA_REF <> '';",
            "SELECT COUNT(*) QSO_CONFIRM_TOTAL, COUNT(DISTINCT DXCC) QSO_DXCC_TOTAL, COUNT(DISTINCT PFX) QSO_PFX_TOTAL FROM logs WHERE QSL_RCVD = 'Y';",
            "SELECT COUNT(DISTINCT CQZ) QSO_CQZ_TOTAL FROM logs WHERE CQZ <> '' AND QSL_RCVD = 'Y';",
            "SELECT COUNT(DISTINCT ITUZ) QSO_ITUZ_TOTAL FROM logs WHERE ITUZ <> '' AND QSL_RCVD = 'Y';",
            "SELECT COUNT(DISTINCT SUBSTR(GRIDSQUARE, 0, 4)) QSO_GRIDSQUARE_TOTAL FROM logs WHERE GRIDSQUARE <> '' AND QSL_RCVD = 'Y';",
            "SELECT STATE, COUNT(*) QSO_PER_STATE FROM logs WHERE STATE <> '' AND DXCC = '291' AND QSL_RCVD = 'Y' GROUP BY STATE ORDER BY STATE;",
            "SELECT STATE, COUNT(*) QSO_PER_STATE FROM logs WHERE STATE <> '' AND DXCC = '1' AND QSL_RCVD = 'Y' GROUP BY STATE ORDER BY STATE;",
            "SELECT COUNTRY, COUNT(*) QSO_PER_COUNTRY FROM logs WHERE COUNTRY <> '' AND QSL_RCVD = 'Y' GROUP BY COUNTRY ORDER BY COUNTRY;",
        ], (sql, callback) => {
            ...
            const [ bands, modes, total_qso, years, months, daysofweek, frequent_flyers, bookends, pota_activations, qro_totals, low_pwr_totals, qrp_totals, qrpp_totals, my_rigs, my_antennas, contest_totals, skcc, pota, confirms, cqz, ituz, grids, states, provs, countries ] = results;
            ...
            const stats = {
                ...
                grids: grids[0],
                enums: {
                    ndxcc: Object.keys(enums.Dxcc).length - 1, // -1 to ignore "0":"None (the contacted station is known to not be within a DXCC entity)"
                },
                states: states,
    ...
    updateSkccRoster(roster, callback) {
        this.#conn.run('DELETE FROM skcc_roster;', [], err => {
            ...
            async.eachSeries(roster.split('\n').map(line => line.split('|')), (member, callback) => {
