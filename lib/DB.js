...
class DB {
    ...
    #createTables(callback) {
        ...
        this.#conn.run(table, [], err => {
            ...
            this.#conn.run(table, [], err => {
                ...
                const table = `
                    CREATE TABLE IF NOT EXISTS super_check_partial (
                        callsign text
                    );
                `;
    ...
    callsignSuggest(partial, callback) {
        partial = `${partial ?? ''}`.toUpperCase() + '%';

        this.#conn.all(`
            SELECT callsign FROM qsos WHERE callsign LIKE ?
                UNION
            SELECT callsign FROM skcc_roster WHERE callsign LIKE ?
                UNION
            SELECT callsign FROM super_check_partial WHERE callsign LIKE ?

            LIMIT ?
        `, [ partial, partial, partial, 10 ], (err, results) => {
            if (err) {
                return callback(err);
            }
            callback(null, results.map(result => result.callsign));
        });
    }

    superCheckPartialUpdate(superCheckPartial, callback) {
        this.#conn.run('DELETE FROM super_check_partial;', [], err => {
            if (err) {
                return callback(err);
            }
            async.eachSeries(superCheckPartial.split('\n').map(line => line.trim()).filter(line => !line.startsWith('#') && line !== ''), (callsign, callback) => {
                this.#conn.run('INSERT INTO super_check_partial (callsign) VALUES (?);', [ callsign ], err => {
                    if (err) {
                        return callback(err);
                    }
                    callback();
