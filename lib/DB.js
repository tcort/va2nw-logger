 ...
 * Ops
 * ---
 * op_id - uniquely identifies this operator - internal use only
 * name - the logging operator's name - ADIF Field: MY_NAME
 * callsign - the logging operator's callsign - ADIF Field: OPERATOR
 */
...
class DB {
    ...
    opInsert(op, callback) {

        const sql = 'INSERT INTO ops (callsign, name) VALUES (?, ?);';
        const vals = [ op.callsign, op.name ];
    ...
    opUpdate(op_id, op, callback) {

        const sql = 'UPDATE ops SET name = ?, callsign = ? WHERE op_id = ?;';
        const vals = [ op.name, op.callsign, op.op_id ];
    ...
    opSelectOne(op_id, callback) {
        const sql = 'SELECT * FROM ops WHERE op_id = ?;';
        const vals = [ op_id ];

        this.#conn.all(sql, vals, (err, ops) => {
            if (err) {
                return callback(err);
            }

            ops = ops.map(op => Op.fromDBO(op));
    ...
    opSelect(callback) {
        const sql = 'SELECT * FROM ops ORDER BY name, callsign;';
        const vals = [ ];
    ...
    opDelete(op_id, callback) {

        const sql = 'DELETE FROM ops WHERE op_id = ?';
        const vals = [ op_id ];
    ...
    qsoCount(query, callback) {
        ...
        if (!(typeof query.since === 'string' && query.since.length > 0)) {
            ...
            if (typeof query.year_since === 'string' && query.year_since.length > 0) {
                query.since = `${query.year_since ?? '1900'}-${query.year_month ?? '01'}-${query.day_since ?? '01'}T${query.hour_since ?? '00'}:${query.minute_since ?? '00'}:${query.second_since ?? '00'}.000Z`;
            }
        }
