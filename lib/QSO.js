'use strict';

const moment = require('moment');

class QSO {

    #qso_id;
    #timeon;
    #timeoff;
    #frequency;
    #mode;
    #power;
    #callsign;
    #rst_sent;
    #rst_rcvd;
    #remarks;
    #station_callsign;
    #operator;
    #name;
    #skcc;
    #fists;
    #spc;
    #contacted_op;
    #pfx;
    #my_spc;
    #dxcc;
    #my_dxcc;
    #country;
    #my_country;
    #gridsquare;
    #my_gridsquare;
    #cq_zone;
    #my_cq_zone;
    #itu_zone;
    #my_itu_zone;
    #qsl_sent;
    #qsl_sent_date;
    #qsl_rcvd;
    #qsl_rcvd_date;
    #sota;
    #my_sota;
    #pota;
    #my_pota;
    #contest;

    constructor(data = {}) {
        this.#qso_id = data?.qso_id;
        this.#timeon = data?.timeon ? new Date(data.timeon) : new Date();
        this.#timeoff = data?.timeoff ? new Date(data.timeoff) : new Date(this.#timeon.toJSON());
        this.#frequency = data?.frequency ?? 0;
        this.#mode = data?.mode ?? 'CW';
        this.#power = data?.power ?? 0;
        this.#callsign = data?.callsign ?? '';
        this.#rst_sent = data?.rst_sent ?? (this.#mode === 'CW' ? '599' : '59');
        this.#rst_rcvd = data?.rst_rcvd ?? (this.#mode === 'CW' ? '599' : '59');
        this.#remarks = data?.remarks ?? '';
        this.#station_callsign = data?.station_callsign ?? '';
        this.#operator = data?.operator ?? '';
        this.#name = data?.name ?? '';
        this.#skcc = data?.skcc ?? '';
        this.#fists = data?.fists ?? '';
        this.#spc = data?.spc ?? '';
        this.#contacted_op = data?.contacted_op ?? '';
        this.#pfx = data?.pfx ?? '';
        this.#my_spc = data?.my_spc ?? '';
        this.#dxcc = data?.dxcc ?? '';
        this.#my_dxcc = data?.my_dxcc ?? '';
        this.#country = data?.country ?? '';
        this.#my_country = data?.my_country ?? '';
        this.#gridsquare = data?.gridsquare ?? '';
        this.#my_gridsquare = data?.my_gridsquare ?? '';
        this.#cq_zone = data?.cq_zone ?? '';
        this.#my_cq_zone = data?.my_cq_zone ?? '';
        this.#itu_zone = data?.itu_zone ?? '';
        this.#my_itu_zone = data?.my_itu_zone ?? '';
        this.#qsl_sent = data?.qsl_sent ?? '';
        this.#qsl_sent_date = data?.qsl_sent_date ?? '';
        this.#qsl_rcvd = data?.qsl_rcvd ?? '';
        this.#qsl_rcvd_date = data?.qsl_rcvd_date ?? '';
        this.#sota = data?.sota ?? '';
        this.#my_sota = data?.my_sota ?? '';
        this.#pota = data?.pota ?? '';
        this.#my_pota = data?.my_pota ?? '';
        this.#contest = data?.contest ?? '';
    }

    toRender() {
        return {
            qso_id: this.#qso_id,
            dateon: moment(this.#timeon).utc().format('YYYY-MM-DD'),
            timeon: moment(this.#timeon).utc().format('HH:mm:ss'),
            dateoff: moment(this.#timeoff).utc().format('YYYY-MM-DD'),
            timeoff: moment(this.#timeoff).utc().format('HH:mm:ss'),
            frequency: this.#frequency.toFixed(3),
            band: this.#freqToBand(this.#frequency),
            mode: this.#mode,
            power: this.#power,
            callsign: this.#callsign,
            rst_sent: this.#rst_sent,
            rst_rcvd: this.#rst_rcvd,
            remarks: this.#remarks,
            station_callsign: this.#station_callsign,
            operator: this.#operator,
            name: this.#name,
            skcc: this.#skcc,
            fists: this.#fists,
            spc: this.#spc,
            contacted_op: this.#contacted_op,
            pfx: this.#pfx,
            my_spc: this.#my_spc,
            dxcc: this.#dxcc,
            my_dxcc: this.#my_dxcc,
            country: this.#country,
            my_country: this.#my_country,
            gridsquare: this.#gridsquare,
            my_gridsquare: this.#my_gridsquare,
            cq_zone: this.#cq_zone,
            my_cq_zone: this.#my_cq_zone,
            itu_zone: this.#itu_zone,
            my_itu_zone: this.#my_itu_zone,
            qsl_sent: this.#qsl_sent,
            qsl_sent_date: this.#qsl_sent_date,
            qsl_rcvd: this.#qsl_rcvd,
            qsl_rcvd_date: this.#qsl_rcvd_date,
            sota: this.#sota,
            my_sota: this.#my_sota,
            pota: this.#pota,
            my_pota: this.#my_pota,
            contest: this.#contest,
        };
    }

    toEdit() {
        return {
            qso_id: this.#qso_id,
            date_on: moment(this.#timeon).utc().format('YYYY-MM-DD'),
            time_on: moment(this.#timeon).utc().format('HH:mm:ss'),
            date_off: moment(this.#timeoff).utc().format('YYYY-MM-DD'),
            time_off: moment(this.#timeoff).utc().format('HH:mm:ss'),
            frequency: this.#frequency.toFixed(3),
            mode: this.#mode,
            power: this.#power,
            callsign: this.#callsign,
            rst_sent: this.#rst_sent,
            rst_rcvd: this.#rst_rcvd,
            remarks: this.#remarks,
            station_callsign: this.#station_callsign,
            operator: this.#operator,
            name: this.#name,
            skcc: this.#skcc,
            fists: this.#fists,
            spc: this.#spc,
            contacted_op: this.#contacted_op,
            pfx: this.#pfx,
            my_spc: this.#my_spc,
            dxcc: this.#dxcc,
            my_dxcc: this.#my_dxcc,
            country: this.#country,
            my_country: this.#my_country,
            gridsquare: this.#gridsquare,
            my_gridsquare: this.#my_gridsquare,
            cq_zone: this.#cq_zone,
            my_cq_zone: this.#my_cq_zone,
            itu_zone: this.#itu_zone,
            my_itu_zone: this.#my_itu_zone,
            qsl_sent: this.#qsl_sent,
            qsl_sent_date: this.#qsl_sent_date,
            qsl_rcvd: this.#qsl_rcvd,
            qsl_rcvd_date: this.#qsl_rcvd_date,
            sota: this.#sota,
            my_sota: this.#my_sota,
            pota: this.#pota,
            my_pota: this.#my_pota,
            contest: this.#contest,
        };
    }

    toJSON() {
        return {
            qso_id: this.#qso_id,
            timeon: this.#timeon.toJSON(),
            timeoff: this.#timeoff.toJSON(),
            frequency: this.#frequency,
            mode: this.#mode,
            power: this.#power,
            callsign: this.#callsign,
            rst_sent: this.#rst_sent,
            rst_rcvd: this.#rst_rcvd,
            remarks: this.#remarks,
            station_callsign: this.#station_callsign,
            operator: this.#operator,
            name: this.#name,
            skcc: this.#skcc,
            fists: this.#fists,
            spc: this.#spc,
            contacted_op: this.#contacted_op,
            pfx: this.#pfx,
            my_spc: this.#my_spc,
            dxcc: this.#dxcc,
            my_dxcc: this.#my_dxcc,
            country: this.#country,
            my_country: this.#my_country,
            gridsquare: this.#gridsquare,
            my_gridsquare: this.#my_gridsquare,
            cq_zone: this.#cq_zone,
            my_cq_zone: this.#my_cq_zone,
            itu_zone: this.#itu_zone,
            my_itu_zone: this.#my_itu_zone,
            qsl_sent: this.#qsl_sent,
            qsl_sent_date: this.#qsl_sent_date,
            qsl_rcvd: this.#qsl_rcvd,
            qsl_rcvd_date: this.#qsl_rcvd_date,
            sota: this.#sota,
            my_sota: this.#my_sota,
            pota: this.#pota,
            my_pota: this.#my_pota,
            contest: this.#contest,
        };
    }

    toDBO() {
        return {
            qso_id: this.#qso_id,
            timeon: this.#timeon.toJSON(),
            timeoff: this.#timeoff.toJSON(),
            frequency: this.#frequency,
            mode: this.#mode,
            power: this.#power,
            callsign: this.#callsign,
            rst_sent: this.#rst_sent,
            rst_rcvd: this.#rst_rcvd,
            remarks: this.#remarks,
            station_callsign: this.#station_callsign,
            operator: this.#operator,
            name: this.#name,
            skcc: this.#skcc,
            fists: this.#fists,
            spc: this.#spc,
            contacted_op: this.#contacted_op,
            pfx: this.#pfx,
            my_spc: this.#my_spc,
            dxcc: this.#dxcc,
            my_dxcc: this.#my_dxcc,
            country: this.#country,
            my_country: this.#my_country,
            gridsquare: this.#gridsquare,
            my_gridsquare: this.#my_gridsquare,
            cq_zone: this.#cq_zone,
            my_cq_zone: this.#my_cq_zone,
            itu_zone: this.#itu_zone,
            my_itu_zone: this.#my_itu_zone,
            qsl_sent: this.#qsl_sent,
            qsl_sent_date: this.#qsl_sent_date,
            qsl_rcvd: this.#qsl_rcvd,
            qsl_rcvd_date: this.#qsl_rcvd_date,
            sota: this.#sota,
            my_sota: this.#my_sota,
            pota: this.#pota,
            my_pota: this.#my_pota,
            contest: this.#contest,
        };
    }

    static fromDBO(dbo) {
        return new QSO({
            qso_id: dbo.qso_id,
            timeon: new Date(dbo.timeon),
            timeoff: new Date(dbo.timeoff),
            frequency: dbo.frequency,
            mode: dbo.mode,
            power: dbo.power,
            callsign: dbo.callsign,
            rst_sent: dbo.rst_sent,
            rst_rcvd: dbo.rst_rcvd,
            remarks: dbo.remarks,
            station_callsign: dbo.station_callsign,
            operator: dbo.operator,
            name: dbo.name,
            skcc: dbo.skcc,
            fists: dbo.fists,
            spc: dbo.spc,
            contacted_op: dbo.contacted_op,
            pfx: dbo.pfx,
            my_spc: dbo.my_spc,
            dxcc: dbo.dxcc,
            my_dxcc: dbo.my_dxcc,
            country: dbo.country,
            my_country: dbo.my_country,
            gridsquare: dbo.gridsquare,
            my_gridsquare: dbo.my_gridsquare,
            cq_zone: dbo.cq_zone,
            my_cq_zone: dbo.my_cq_zone,
            itu_zone: dbo.itu_zone,
            my_itu_zone: dbo.my_itu_zone,
            qsl_sent: dbo.qsl_sent,
            qsl_sent_date: dbo.qsl_sent_date,
            qsl_rcvd: dbo.qsl_rcvd,
            qsl_rcvd_date: dbo.qsl_rcvd_date,
            sota: dbo.sota,
            my_sota: dbo.my_sota,
            pota: dbo.pota,
            my_pota: dbo.my_pota,
            contest: dbo.contest,
        });
    }

    toAO(auxData = {}) {
        return Object.assign({}, auxData, {
            QSO_DATE: moment(this.#timeon).utc().format('YYYYMMDD'),
            TIME_ON: moment(this.#timeon).utc().format('HHmmss'),
            QSO_DATE_OFF: moment(this.#timeoff).utc().format('YYYYMMDD'),
            TIME_OFF: moment(this.#timeoff).utc().format('HHmmss'),
            FREQ: `${this.#frequency}`,
            BAND: this.#freqToBand(this.#frequency),
            MODE: this.#mode,
            TX_PWR: `${this.#power}`,
            CALL: this.#callsign,
            RST_SENT: `${this.#rst_sent}`,
            RST_RCVD: `${this.#rst_rcvd}`,
            NOTES: this.#remarks,
            STATION_CALLSIGN: this.#station_callsign,
            OPERATOR: this.#operator,
            NAME: this.#name,
            SKCC: this.#skcc,
            FISTS: this.#fists,
            STATE: this.#spc,
            CONTACTED_OP: this.#contacted_op,
            PFX: this.#pfx,
            MY_STATE: this.#my_spc,
            DXCC: this.#dxcc,
            MY_DXCC: this.#my_dxcc,
            COUNTRY: this.#country,
            MY_COUNTRY: this.#my_country,
            GRIDSQUARE: this.#gridsquare,
            MY_GRIDSQUARE: this.#my_gridsquare,
            CQZ: this.#cq_zone,
            MY_CQZ: this.#my_cq_zone,
            ITUZ: this.#itu_zone,
            MY_ITUZ: this.#my_itu_zone,
            QSL_SENT: this.#qsl_sent,
            QSLSDATE: this.#qsl_sent_date,
            QSL_RCVD: this.#qsl_rcvd,
            QSLRDATE: this.#qsl_rcvd_date,
            SOTA_REF: this.#sota,
            MY_SOTA_REF: this.#my_sota,
            POTA_REF: this.#pota,
            MY_POTA_REF: this.#my_pota,
            CONTEST_ID: this.#contest,
        });
    }

    static fromAOtoJSON(ao) {
        const json = {};

        if (ao.QSO_DATE && ao.TIME_ON) {
            json.timeon = QSO.#createTimestamp(ao.QSO_DATE, ao.TIME_ON);
        }

        if (ao.QSO_DATE_OFF && ao.TIME_OFF) {
            json.timeoff = QSO.#createTimestamp(ao.QSO_DATE_OFF ? ao.QSO_DATE_OFF : ao.QSO_DATE, ao.TIME_OFF ? ao.TIME_OFF : ao.TIME_ON);
        }

        [
            [ 'FREQ', 'frequency' ],
            [ 'MODE', 'mode' ],
            [ 'TX_PWR', 'power' ],
            [ 'CALL', 'callsign' ],
            [ 'RST_SENT', 'rst_sent' ],
            [ 'RST_RCVD', 'rst_rcvd' ],
            [ 'NOTES', 'remakrs' ],
            [ 'STATION_CALLSIGN', 'station_callsign' ],
            [ 'OPERATOR', 'operator' ],
            [ 'NAME', 'name' ],
            [ 'SKCC', 'skcc' ],
            [ 'FISTS', 'fists' ],
            [ 'STATE', 'spc' ],
            [ 'CONTACTED_OP', 'contacted_op' ],
            [ 'PFX', 'pfx' ],
            [ 'MY_STATE', 'my_spc' ],
            [ 'DXCC', 'dxcc' ],
            [ 'MY_DXCC', 'my_dxcc' ],
            [ 'COUNTRY', 'country' ],
            [ 'MY_COUNTRY', 'my_country' ],
            [ 'GRIDSQUARE', 'gridsquare' ],
            [ 'MY_GRIDSQUARE', 'my_gridsquare' ],
            [ 'CQZ', 'cq_zone' ],
            [ 'MY_CQZ', 'my_cq_zone' ],
            [ 'ITUZ', 'itu_zone' ],
            [ 'MY_ITUZ', 'my_itu_zone' ],
            [ 'QSL_SENT', 'qsl_sent' ],
            [ 'QSLSDATE', 'qsl_sent_date' ],
            [ 'QSL_RCVD', 'qsl_rcvd' ],
            [ 'QSLRDATE', 'qsl_rcvd_date' ],
            [ 'SOTA_REF', 'sota' ],
            [ 'MY_SOTA_REF', 'my_sota' ],
            [ 'POTA_REF', 'pota' ],
            [ 'MY_POTA_REF', 'my_pota' ],
            [ 'CONTEST_ID', 'contest' ],
        ].forEach(([ aoName, qsoName ]) => {
            if (ao[aoName]) {
                json[qsoName] = ao[aoName];
            }
        });

        return json;
    }

    static fromAO(ao) {
        return new QSO({
            timeon: QSO.#createTimestamp(ao.QSO_DATE, ao.TIME_ON),
            timeoff: QSO.#createTimestamp(ao.QSO_DATE_OFF ? ao.QSO_DATE_OFF : ao.QSO_DATE, ao.TIME_OFF ? ao.TIME_OFF : ao.TIME_ON),
            frequency: ao.FREQ ? parseFloat(ao.FREQ) : this.#bandToFreq(ao.BAND),
            mode: ao.MODE,
            power: isNaN(parseFloat(ao.TX_PWR)) ? 0 : parseFloat(ao.TX_PWR),
            callsign: ao.CALL,
            rst_sent: isNaN(parseInt(ao.RST_SENT)) ? (ao.MODE === 'CW' ? '599' : '59') : parseInt(ao.RST_SENT),
            rst_rcvd: isNaN(parseInt(ao.RST_RCVD)) ? (ao.MODE === 'CW' ? '599' : '59') : parseInt(ao.RST_RCVD),
            remarks: ao.NOTES ?? '',
            station_callsign: ao.STATION_CALLSIGN,
            operator: ao.OPERATOR,
            name: ao.NAME,
            skcc: ao.SKCC,
            fists: ao.FISTS,
            spc: ao.STATE,
            contacted_op: ao.CONTACTED_OP,
            pfx: ao.PFX,
            my_spc: ao.MY_STATE,
            dxcc: ao.DXCC,
            my_dxcc: ao.MY_DXCC,
            country: ao.COUNTRY,
            my_country: ao.MY_COUNTRY,
            gridsquare: ao.GRIDSQUARE,
            my_gridsquare: ao.MY_GRIDSQUARE,
            cq_zone: ao.CQZ,
            my_cq_zone: ao.MY_CQZ,
            itu_zone: ao.ITUZ,
            my_itu_zone: ao.MY_ITUZ,
            qsl_sent: ao.QSL_SENT,
            qsl_sent_date: ao.QSLSDATE,
            qsl_rcvd: ao.QSL_RCVD,
            qsl_rcvd_date: ao.QSLRDATE,
            sota: ao.SOTA_REF,
            my_sota: ao.MY_SOTA_REF,
            pota: ao.POTA_REF,
            my_pota: ao.MY_POTA_REF,
            contest: ao.CONTEST_ID,
        });
    }

    static #createTimestamp(DATE, TIME) {
        const year = DATE.slice(0, 4);
        const month = DATE.slice(4, 6);
        const day = DATE.slice(6, 8);

        TIME = TIME.length === 4 ? `${TIME}00` : TIME; /* normalize to 6 digit time */

        const hour = TIME.slice(0, 2);
        const minute = TIME.slice(2, 4);
        const second = TIME.slice(4, 6);

        return `${year}-${month}-${day}T${hour}:${minute}:${second}.000Z`;
    }

    #bandToFreq(BAND) {
        BAND = `${BAND}`.trim().toLowerCase();

        switch (BAND) {
            case '2190m': return 0.1357;
            case '630m': return 0.472;
            case '560m': return 0.501;
            case '160m': return 1.8;
            case '80m': return 3.5;
            case '60m': return 5.06;
            case '40m': return 7;
            case '30m': return 10.1;
            case '20m': return 14;
            case '17m': return 18.068;
            case '15m': return 21;
            case '12m': return 24.89;
            case '10m': return 28;
            case '8m': return 40;
            case '6m': return 50;
            case '5m': return 54.000001;
            case '4m': return 70;
            case '2m': return 144;
            case '1.25m': return 222;
            case '70cm': return 420;
            case '33cm': return 902;
            case '23cm': return 1240;
            case '13cm': return 2300;
            case '9cm': return 3300;
            case '6cm': return 5650;
            case '3cm': return 10000;
            case '1.25cm': return 24000;
            case '6mm': return 47000;
            case '4mm': return 75500;
            case '2.5mm': return 119980;
            case '2mm': return 134000;
            case '1mm': return 241000;
            case 'submm': return 300000;
            default: return 0;
        };
    }

    #freqToBand(freq) {
        freq = parseFloat(freq);

        const result = [
            { band: "2190m", min: .1357, max: .1378 },
            { band: "630m", min: .472, max: .479 },
            { band: "560m", min: .501, max: .504 },
            { band: "160m", min: 1.8, max: 2.0 },
            { band: "80m", min: 3.5, max: 4.0 },
            { band: "60m", min: 5.06, max: 5.45 },
            { band: "40m", min: 7.0, max: 7.3 },
            { band: "30m", min: 10.1, max: 10.15 },
            { band: "20m", min: 14.0, max: 14.35 },
            { band: "17m", min: 18.068, max: 18.168 },
            { band: "15m", min: 21.0, max: 21.45 },
            { band: "12m", min: 24.890, max: 24.99 },
            { band: "10m", min: 28.0, max: 29.7 },
            { band: "8m", min: 40, max: 45 },
            { band: "6m", min: 50, max: 54 },
            { band: "5m", min: 54.000001, max: 69.9 },
            { band: "4m", min: 70, max: 71 },
            { band: "2m", min: 144, max: 148 },
            { band: "1.25m", min: 222, max: 225 },
            { band: "70cm", min: 420, max: 450 },
            { band: "33cm", min: 902, max: 928 },
            { band: "23cm", min: 1240, max: 1300 },
            { band: "13cm", min: 2300, max: 2450 },
            { band: "9cm", min: 3300, max: 3500 },
            { band: "6cm", min: 5650, max: 5925 },
            { band: "3cm", min: 10000, max: 10500 },
            { band: "1.25cm", min: 24000, max: 24250 },
            { band: "6mm", min: 47000, max: 47200 },
            { band: "4mm", min: 75500, max: 81000 },
            { band: "2.5mm", min: 119980, max: 123000 },
            { band: "2mm", min: 134000, max: 149000 },
            { band: "1mm", min: 241000, max: 250000 },
            { band: "submm", min: 300000, max: 7500000 },
        ].find(({ min, max, band }) => (min <= freq && freq <= max));

        return result?.band;
    }

}

module.exports = QSO;
