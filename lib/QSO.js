...
class QSO {
    ...
    toAO(auxData = {}) {
        return Object.assign({}, auxData, {
            ...
            SKCC: this.#skcc,
            STATE: this.#spc,
            PFX: this.#callToPfx(this.#callsign),
            CONTACTED_OP: this.#contacted_op,
        });
    }
    ...
    #callToPfx(CALL) {
        CALL = `${CALL}`.trim().toUpperCase().split('/').sort((a,b) => b.length - a.length)[0];
        return CALL.slice(0, CALL.length - CALL.split('').reverse().findIndex(ch => /[0-9]/.test(ch)));
    }
