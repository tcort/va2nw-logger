...
function routerFactory(options = {}) {
            ...
            req.db.qsoSelect(Object.assign({ order: 'asc', page, pageSize }, req.query), (err, qsos) => {
                ...
                switch (req.query.fmt) {
                    ...
                    default:
                        ...
                        res.render('qso-list', { qsos, pageNav }, function (err, html) {
                            if (err) {
                                return next(err);
                            }
                            res.send(html);
    ...
    router.get('/qsos/:qso_id', function (req, res, next) {
        req.db.qsoSelectOne(req.params.qso_id, (err, qso) => {
            ...
            res.render(req.query.form === 'edit' ? 'qso-update' : 'qso-edit', { qso }, function (err, html) {
                if (err) {
                    return next(err);
                }
                res.send(html);
    ...
    router.get('/ops/new', (req, res, next) => {
        res.render('op-create', { }, function (err, html) {
            if (err) {
                return next(err);
            }
            res.send(html);
    ...
    router.get('/ops', function (req, res, next) {
        req.db.opSelect((err, ops) => {
            ...
            switch (req.query.fmt) {
                ...
                case 'json':
                    res.json(ops.map(op => op.toJSON()));
                    req.db.close();
                    break;

                default:

                    ops = ops.map(op => op.toRender());

                    res.render('op-list', { ops }, function (err, html) {
                        if (err) {
                            return next(err);
                        }
                        res.send(html);
    ...
    router.get('/ops/:op_id', function (req, res, next) {
        req.db.opSelectOne(req.params.op_id, (err, op) => {
            if (err) {
                return next(err);
            }
            ...
            res.render(req.query.form === 'edit' ? 'op-update' : 'op-read', { op }, function (err, html) {
                if (err) {
                    return next(err);
                }
                res.send(html);
    ...
    router.put('/ops/:op_id', function (req, res, next) {

        req.db.opSelectOne(req.params.op_id, (err, originalOp) => {
            if (err) {
                return next(err);
            }

            const inputOp = new Op({
                name: req.body.name,
                callsign: req.body.callsign,
            });

            // default values from existing op
            const op = _.defaultsDeep(inputOp.toDBO(), originalOp.toDBO());

            // update db record
            req.db.opUpdate(req.params.op_id, op, (err) => {
                if (err) {
                    return next(err);
                }
                res.redirect('/ops');
    ...
    router.delete('/ops/:op_id', function (req, res, next) {
        req.db.opDelete(req.params.op_id, (err) => {
            if (err) {
                return next(err);
            }
            res.redirect('/ops');
