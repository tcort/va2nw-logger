...
router.post('/logs', function (req, res, next) {
    ...
    if (typeof req.body?.date_on === 'string' && req.body.date_on.length > 0) {
        req.body.QSO_DATE = `${req.body.date_on}`.replace(/[^0-9]/g, '');
    }
    if (typeof req.body?.time_on === 'string' && req.body.time_on.length > 0) {
        req.body.TIME_ON = `${req.body.time_on}`.replace(/[^0-9]/g, '');
    }
    if (typeof req.body?.date_off === 'string' && req.body.date_off.length > 0) {
        req.body.QSO_DATE_OFF = `${req.body.date_off}`.replace(/[^0-9]/g, '');
    }
    if (typeof req.body?.time_off === 'string' && req.body.time_off.length > 0) {
        req.body.TIME_OFF = `${req.body.time_off}`.replace(/[^0-9]/g, '');
    }
    ...
    req.db.insert(qso, err => {
        ...
        res.format({
            json: () => res.status(201).json(qso),
            html: () => res.redirect('/logs/new'),
...
router.get('/logs/new', function (req, res, next) {
    res.render('create', { enums }, function (err, html) {
        if (err) {
            return next(err);
        }
        res.send(html);
...
router.get('/logs/export', function (req, res, next) {
    ...
    req.db.stationCallsigns(function (err, station_callsigns) {
        if (err) {
            return next(err);
        }
        res.render('export', { station_callsigns }, function (err, html) {
            if (err) {
                return next(err);
            }
            res.send(html);
...
router.get('/logs/import', function (req, res, next) {
    res.render('import', { }, function (err, html) {
        if (err) {
            return next(err);
        }
        res.send(html);
...
router.get('/logs', function (req, res, next) {
    ...
    req.db.selectCount(req.query, (err, count) => {
        if (err) {
            return next(err);
        }
        req.db.stationCallsigns(function (err, station_callsigns) {
            ...
            req.db.select(Object.assign({ order: 'asc', page, pageSize }, req.query), (err, logs) => {
                if (err) {
                    return next(err);
                }
                ...
                switch (req.query.fmt) {
                    ...
                    default:
                        ...
                        const pageNav = [];
                        const query = [ 'callsign', 'station_callsign' ]
                                            .filter(prop => typeof req.query[prop] === 'string' && req.query[prop].length > 0)
                                            .reduce((result, prop) => {
                        ...
                        res.render('logs', { pageNav, currentPage, npages, count, callsign: req.query.callsign ?? undefined, station_callsigns, logs, enums }, function (err, html) {
                            if (err) {
                                return next(err);
                            }
                            res.send(html);
