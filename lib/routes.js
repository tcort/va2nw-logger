...
function routerFactory(options = {}) {
    ...
    router.use('/bulma', express.static(path.join(__dirname, '..', 'node_modules', 'bulma'), { etag: false }));
   ...
   router.get('/skcc/:callsign', function (req, res, next) {
    ...
    router.get('/qsos/new', (req, res, next) => {
        if (!['default','skcc','contest','sst'].includes(req.query.logger_mode)) {
            res.redirect('/qsos/new?' + qs.stringify({ logger_mode: 'default' }));
            req.db.close();
            return;
        }
        res.render('qso-create', { [`logger_mode_${req.query.logger_mode}`]: true, logger_mode: req.query.logger_mode }, function (err, html) {
            if (err) {
                return next(err);
            }
            res.send(html);
    ...
    router.post('/qsos', (req, res, next) => {
        ...
        if (req.is('multipart/form-data')) {
            ...
            reader.on('data', qso => {
                ...
                if (qso.CONTEST_ID) { additionalRemarks.push(qso.CONTEST_ID); }
                ...
                if (qso.FISTS) { additionalRemarks.push(`FISTS=${qso.FISTS}`); }
                if (qso.POTA) { additionalRemarks.push(`POTA=${qso.POTA}`); }
                if (qso.SOTA) { additionalRemarks.push(`POTA=${qso.POTA}`); }
                if (qso.MY_POTA) { additionalRemarks.push(`POTA@${qso.MY_POTA}`); }
                if (qso.MY_SOTA) { additionalRemarks.push(`SOTA@${qso.MY_SOTA}`); }
                ...
                if (qso.CONTACTED_OP) { additionalRemarks.push(`CONTACTED_OP=${qso.CONTACTED_OP}`); }
        ...
        req.db.qsoInsert(qso.toDBO(), err => {
            ...
            res.redirect('/qsos/new?' + qs.stringify({ logger_mode: req.query.logger_mode }));
            req.db.close();
        });
    });
