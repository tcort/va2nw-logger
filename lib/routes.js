...
function routerFactory(options = {}) {
    ...
    router.get('/qsos', function (req, res, next) {
        ...
        if (typeof req?.query?.mypota === 'string' && req.query.mypota.length > 0) {
            auxData.MY_SIG = 'POTA';
            auxData.MY_SIG_INFO = req.query.mypota;
            auxData.MY_POTA_REF = req.query.mypota;
        } else if (typeof req?.query?.mysota === 'string' && req.query.mysota.length > 0) {
            auxData.MY_SIG = 'SOTA';
            auxData.MY_SIG_INFO = req.query.mysota;
            auxData.MY_SOTA_REF = req.query.mysota;
        }
