...
function routerFactory(options = {}) {
   ...
   router.get('/skcc/:callsign', function (req, res, next) {
    ...
    router.get('/qsos/new', (req, res, next) => {
        if (!['default','skcc','contest','sst','ota'].includes(req.query.logger_mode)) {
    ...
    router.get('/callsigns', (req, res, next) => {
        req.db.callsignSuggest(req.query.partial, (err, callsigns) => {
            if (err) {
                return next(err);
            }
            res.json(callsigns);
