            ...
            req.db.qsoSelect(Object.assign({ order: 'asc', page, pageSize }, req.query), (err, qsos) => {
                ...
                switch (req.query.fmt) {
                    ...
                    default:
                        ...
                        const query = (typeof req.query.callsign === 'string' && req.query.callsign.length > 0) ? { callsign: req.query.callsign } : {};
                        ...
                        res.render('qso-list', { qsos, pageNav, currentPage, npages, count, callsign: query.callsign ?? '' }, function (err, html) {
                            if (err) {
                                return next(err);
                            }
                            res.send(html);
