...
function routerFactory(options = {}) {
    ...
    const VALID_INPUT_MODES = ['default','sst','sota','pota','pota_skcc','skcc'];

    router.get('/qsos/new', (req, res, next) => {

        const locals = { VALID_INPUT_MODES };

        if (VALID_INPUT_MODES.includes(req.query.input_mode)) {
            locals[`input_mode_${req.query.input_mode}`] = true;
        } else {
            locals.input_mode_default = true;
        }

        res.render('qso-create', locals, function (err, html) {
            if (err) {
                return next(err);
            }
            res.send(html);
    ...
    router.post('/qsos', (req, res, next) => {
        ...
        const input_mode = VALID_MODES_INPUT.includes(req.query.input_mode) ? req.query.input_mode : 'default';
        ...
        if (req.is('multipart/form-data')) {
            ...
            reader.on('finish', () => {
                (function checkComplete() {
                    if (in_flight === 0) {
                        res.redirect('/qsos/new?' + qs.stringify({ input_mode }));
                        req.db.close();
                    } else {
