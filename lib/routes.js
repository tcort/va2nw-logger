    ...
    router.post('/qsos', (req, res, next) => {
        ...
        if (req.is('multipart/form-data')) {
            ...
            reader.on('data', qso => {
                ...
                if (qso.STATE) { additionalRemarks.push(`STATE=${qso.STATE}`); }
                if (qso.NAME) { additionalRemarks.push(`NAME=${qso.NAME}`); }
                if (qso.SKCC) { additionalRemarks.push(`SKCC=${qso.SKCC}`); }
                if (qso.POTA) { additionalRemarks.push(`POTA_REF=${qso.POTA}`); }
                if (qso.SOTA) { additionalRemarks.push(`SOTA_REF=${qso.POTA}`); }
                if (qso.MY_POTA) { additionalRemarks.push(`MY_POTA_REF=${qso.MY_POTA}`); }
                if (qso.MY_SOTA) { additionalRemarks.push(`MY_SOTA_REF=${qso.MY_SOTA}`); }
