...
router.get('/logs', function (req, res, next) {
    ...
    req.db.rigs((err, rigs) => {
        if (err) {
            return next(err);
        }

        res.locals.rigs = rigs;
        next();
    });

}, function (req, res, next) {

    req.db.antennas((err, antennas) => {
        if (err) {
            return next(err);
        }

        res.locals.antennas = antennas;
        next();
    });
    ...
    req.db.select(req.query, (err, logs) => {
        ...
        switch (req.query.fmt) {
            ...
            case 'json':
                ...
                break;
            default:
                req.db.stats((err, stats) => {
                    if (err) {
                        return next(err);
                    }
                    res.render('logs', { logs, stats }, function (err, html) {
                        if (err) {
                            return next(err);
                        }
                        res.send(html);
